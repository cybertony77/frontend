...
import { useStudents, useStudent, useToggleAttendance, useUpdateHomework, useUpdateQuizGrade } from "../../lib/api/students";
import HomeworkSelect from "../../components/HomeworkSelect";
...
export default function QR() {
  ...
  const [homeworkStatus, setHomeworkStatus] = useState(''); // 'Done', 'Not Completed', 'Not Done', 'No Homework', or '' for no selection
  ...
  const getCurrentHomeworkStatus = () => {
    ...
    // If it's a string (new format), return it directly
    if (typeof currentHwDone === 'string') {
      return currentHwDone;
    }
    
    // If it's null (no data), return empty string for "H.W: ..." display
    ...
    return ""; // Return empty string for default state (shows "H.W: ...")
  };
  ...
  useEffect(() => {
    ...
    if (currentAttended === false) {
      // If attendance is false, reset homework to null (empty string for "H.W: ..." display)
      setOptimisticHwDone(null);
      setHomeworkStatus(''); // Clear homework selection to show "H.W: ..."
  ...
  const handleHomeworkStatusChange = async (status) => {
    if (!student || !selectedWeek || !attendanceCenter) return;
    ...
    updateHomeworkMutation.mutate({
      id: student.id,
      homeworkData: { hwStatus: status, week: weekNumber }
    });
  };
        ...
        <div className="student-card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ width: '100%' }}>
                <HomeworkSelect
                  selectedStatus={getCurrentHomeworkStatus()}
                  ...
                  isOpen={openDropdown === 'homework'}
                  onToggle={() => setOpenDropdown(openDropdown === 'homework' ? null : 'homework')}
