...
export default async function handler(req, res) {
  const { id } = req.query;
  // Convert ID to number for consistency (all IDs are now stored as numbers)
  ...
  try {
    ...
    if (req.method === 'GET') {
      // Get student info - all IDs are now stored as numbers
      console.log('🔍 Searching for student with ID:', student_id, 'Type:', typeof student_id);
      ...
      // Update student - all IDs are now stored as numbers
      ...
      // Delete student - all IDs are now stored as numbers
